...
export default function SobreModal({
  ...
  const rareza = cromo
    ? RAREZA_CONFIG[cromo.stickers?.rareza as keyof typeof RAREZA_CONFIG] ||
      RAREZA_CONFIG.comun
    : RAREZA_CONFIG.comun;
  ...
  const cardOpacity = flipAnim.interpolate({
    inputRange: [0, 0.3, 1],
    outputRange: [0.4, 0.8, 1], // Empieza al 40% y sube rápido al 80%
  });
      ...
      <Animated.View style={[s.overlay, { opacity: opacityAnim }]}>
        ...
        {paso === "cromos" && cromo && (
          <View style={s.cromoContainer}>
            <Text style={s.cromoCounter}>
              {cromoActual + 1} / {cromos.length}
            </Text>
            ...
            >
              ...
              <View style={[s.cromoBadge, { backgroundColor: rareza.bg }]}>
                <Text style={[s.cromoBadgeText, { color: rareza.color }]}>
                  {rareza.label}
                </Text>
              </View>
