...
export default function PremiosScreen() {
  ...
  const fetchAndSegmentar = useCallback(async () => {
    ...
    try {
      ...
      const tierNombre =
        (userData.economic_tiers as any)?.nombre || medallaALigaTier(medallaFinal);
      ...
      const payload = {
        ...
        frecuencia_transacciones_mes: userData.frecuencia_transacciones_mes || 0,
        ...
        predicciones_correctas_pct: Number(userData.predicciones_correctas_pct) || 0,
        ...
        rol_en_grupo: userData.rol_en_grupo === 'ninguno' ? 'sin_grupo' : (userData.rol_en_grupo || 'sin_grupo'),
  ...
  const categoriaColor = result ? (CATEGORIA_COLOR[result.categoria] ?? '#b2c5ff') : '#b2c5ff';
      ...
      >
        ...
        {loading && (
          <View style={s.stateContainer}>
            <ActivityIndicator size="large" color="#b2c5ff" style={{ marginBottom: 16 }} />
            <Text style={s.stateTitle}>Analizando tu perfil…</Text>
            <Text style={s.stateSub}>El modelo está personalizando tu premio</Text>
          </View>
        )}
        ...
        {!loading && error && (
          <View style={s.stateContainer}>
            ...
            <Text style={[s.stateSub, { marginTop: 12, color: '#8c90a1' }]}>
              ...
              <Text style={{ color: '#b2c5ff' }}>uvicorn api:app --reload --port 8000</Text>
            </Text>
        ...
        {!loading && !error && result && (
          <>
            ...
            <View style={[s.prizeCard, { borderColor: categoriaColor }]}>
              <View style={[s.prizeCardGlow, { backgroundColor: categoriaColor + '18' }]} />

              {/* Badge de categoría */}
              <View style={[s.categoryBadge, { backgroundColor: categoriaColor + '22', borderColor: categoriaColor + '55' }]}>
                <Text style={s.categoryBadgeIcon}>{CATEGORIA_ICON[result.categoria] ?? '🎁'}</Text>
              ...
              {/* Premio principal */}
              <Text style={s.prizeEmoji}>{CATEGORIA_ICON[result.categoria] ?? '🎁'}</Text>
              <Text style={s.prizeName}>{result.premio_nombre}</Text>

              {/* Métricas */}
              <View style={s.metricsRow}>
                <View style={s.metricItem}>
                  <Text style={[s.metricValue, { color: categoriaColor }]}>{result.afinidad_pct}%</Text>
                  <Text style={s.metricLabel}>Afinidad</Text>
                </View>
                <View style={s.metricDivider} />
                <View style={s.metricItem}>
                  <Text style={[s.metricValue, { color: categoriaColor }]}>{result.confianza_pct}%</Text>
              ...
              <View style={s.reasonsList}>
                {result.razones.map((r, i) => (
                  <View key={i} style={s.reasonItem}>
                    <View style={[s.reasonDot, { backgroundColor: categoriaColor }]} />
                ...
                return (
                  <View key={cat} style={s.catRow}>
                    ...
                    <View style={s.catBarContainer}>
                      <Text style={s.catLabel}>{CATEGORIA_LABEL[cat] ?? cat}</Text>
              ...
              <View style={s.section}>
                ...
                {result.alternativas.map((alt) => (
                  <View key={alt.premio} style={s.altItem}>
                    <Text style={s.altEmoji}>{CATEGORIA_ICON[result.categoria] ?? '🎁'}</Text>
                    <View style={s.altInfo}>
                      <Text style={s.altName}>{alt.nombre}</Text>
                      <Text style={s.altAfinidad}>{alt.afinidad}% de afinidad</Text>
                    </View>
                    <View style={[s.altBadge, { borderColor: categoriaColor + '55' }]}>
                      <Text style={[s.altBadgeText, { color: categoriaColor }]}>Alt.</Text>
                    </View>
                  </View>
                ))}
              </View>
...
const s = StyleSheet.create({
  ...
  backBtn: {
    ...
    backgroundColor: '#1f2a3d', alignItems: 'center', justifyContent: 'center',
  ...
  tierBadge: {
    backgroundColor: '#1f2a3d', paddingHorizontal: 10, paddingVertical: 5,
    borderRadius: 20, borderWidth: 0.5, borderColor: '#ffd65b55',
  },
  ...
  stateTitle: { color: '#d7e3fc', fontSize: 18, fontWeight: '700', marginBottom: 8, textAlign: 'center' },
  stateSub: { color: '#8c90a1', fontSize: 13, textAlign: 'center', lineHeight: 20 },
  retryBtn: {
    marginTop: 24, backgroundColor: '#b2c5ff', paddingHorizontal: 28,
  ...
  metricsRow: {
    ...
    backgroundColor: '#0d1829', borderRadius: 16, paddingVertical: 14,
    paddingHorizontal: 24, width: '100%', justifyContent: 'center',
  },
  metricItem: { alignItems: 'center', flex: 1 },
  metricValue: { fontSize: 26, fontWeight: '900' },
  metricLabel: { color: '#8c90a1', fontSize: 10, fontWeight: '600', marginTop: 2, textTransform: 'uppercase', letterSpacing: 0.8 },
  metricDivider: { width: 1, height: 36, backgroundColor: '#1f2a3d', marginHorizontal: 12 },

  // Secciones
  section: {
    backgroundColor: '#101c2e', borderRadius: 20, padding: 18,
  ...
  catLabel: { color: '#c2c6d8', fontSize: 12, fontWeight: '600', marginBottom: 4 },
  catBarTrack: { height: 6, backgroundColor: '#1f2a3d', borderRadius: 3, overflow: 'hidden' },
  ...
  altItem: {
    ...
    paddingVertical: 12, borderBottomWidth: 0.5, borderBottomColor: '#1f2a3d',
  },
  ...
  bottomNav: {
    ...
    backgroundColor: 'rgba(7,19,37,0.95)', flexDirection: 'row',
  ...
  navCenterBtn: {
    width: 56, height: 56, borderRadius: 28, backgroundColor: '#b2c5ff',
    alignItems: 'center', justifyContent: 'center', marginBottom: 4,
    borderWidth: 3, borderColor: '#071325',
  },
