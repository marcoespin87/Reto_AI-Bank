...
export default function AlbumScreen() {
  ...
  const RAREZA_CONFIG = {
    epico: { color: colors.rarityEpicText, border: colors.rarityEpicBorder, label: 'ÉPICO',  bg: colors.rarityEpicBg },
    raro:  { color: colors.rarityRareText, border: colors.rarityRareBorder, label: 'RARO',   bg: colors.rarityRareBg },
    comun: { color: colors.rarityCommonText, border: colors.rarityCommonBorder, label: 'COMÚN', bg: colors.rarityCommonBg },
  };
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.filtrosRow}>
          {(['todos', 'epico', 'raro', 'comun'] as const).map((f) => {
            ...
            return (
              <TouchableOpacity
                key={f}
                style={[s.filtroBtn, isActive && { backgroundColor: rConfig?.border || colors.primary, borderColor: rConfig?.border || colors.primary }]}
                onPress={() => setFiltro(f)}
              >
                <Text style={[s.filtroBtnText, isActive && s.filtroBtnTextActive]}>
              ...
              >
                {lo_tengo ? (
                  <>
                    ...
                    <View style={[s.rarezaBadge, { backgroundColor: rareza.bg, borderColor: rareza.border, borderWidth: 0.5 }]}>
            ...
            return (
              <View style={[s.modalCard, { borderColor: rareza.border }]}>
                ...
                <View style={s.modalInfo}>
                  ...
                  {cantidad > 1 && <Text style={s.modalCantidad}>Tienes {cantidad} copias de este cromo</Text>}
...
function getStyles(colors: ReturnType<typeof import('../../context/ThemeContext').useTheme>['colors']) {
  return StyleSheet.create({
    ...
    leagueBadge: { backgroundColor: colors.cardBackground, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, borderWidth: 0.5, borderColor: colors.borderStrong },
    leagueBadgeText: { color: colors.primary, fontSize: 10, fontWeight: '700' },

    goldenBanner: { backgroundColor: colors.goldDim, borderRadius: 16, padding: 16, marginBottom: 16, flexDirection: 'row', alignItems: 'center', gap: 12, borderWidth: 1, borderColor: colors.goldBorder },
    ...
    progresoNum: {
      color: colors.gold, fontSize: 40, fontWeight: '800', letterSpacing: -1,
      textShadowColor: colors.goldShadow,
      textShadowOffset: { width: 0, height: 2 },
      textShadowRadius: 6,
    },
    progresoTotal: { color: colors.textSecondary, fontSize: 16, fontWeight: '500' },
    progressBar: { height: 8, backgroundColor: colors.borderMedium, borderRadius: 4, overflow: 'hidden', marginBottom: 8 },
    progressFill: { height: '100%', backgroundColor: colors.primary, borderRadius: 4 },
    progresoSub: { color: colors.textSecondary, fontSize: 12, marginBottom: 8 },
    infoBox: { backgroundColor: colors.backgroundSecondary, borderRadius: 10, padding: 10, borderWidth: 0.5, borderColor: colors.borderMedium },
    infoText: { color: colors.primary, fontSize: 11, fontWeight: '600' },

    filtrosRow: { marginBottom: 16 },
    filtroBtn: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, backgroundColor: colors.backgroundSecondary, marginRight: 8, borderWidth: 0.5, borderColor: colors.borderMedium },
    filtroBtnText: { color: colors.textSecondary, fontSize: 12, fontWeight: '600' },
    filtroBtnTextActive: { color: '#fff', fontWeight: '800' },

    grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
    stickerCard: { width: '30%', aspectRatio: 3 / 4, borderRadius: 12, borderWidth: 1.5, overflow: 'hidden', position: 'relative', backgroundColor: colors.backgroundSecondary },
    stickerCardLocked: { opacity: 0.35 },
    stickerImage: { width: '100%', height: '100%' },
    rarezaBadge: { position: 'absolute', bottom: 4, left: 4, right: 4, paddingVertical: 3, borderRadius: 6, alignItems: 'center' },
    rarezaBadgeText: { fontSize: 8, fontWeight: '800', letterSpacing: 0.5 },
    cantidadBadge: { position: 'absolute', top: 4, right: 4, backgroundColor: colors.gold, borderRadius: 10, paddingHorizontal: 5, paddingVertical: 2 },
    cantidadText: { color: colors.textOnGold, fontSize: 9, fontWeight: '800' },
    stickerLocked: { flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.cardBackground },
    stickerLockedText: { color: colors.textMuted, fontSize: 32, fontWeight: '800' },

    modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.85)', alignItems: 'center', justifyContent: 'center', padding: 24 },
    modalCard: { backgroundColor: colors.cardBackgroundAlt, borderRadius: 24, overflow: 'hidden', borderWidth: 2, width: '80%' },
    ...
    modalNombre: { color: colors.textPrimary, fontSize: 18, fontWeight: '800', textAlign: 'center', marginBottom: 4 },
    modalCantidad: { color: colors.gold, fontSize: 12, fontWeight: '600', marginBottom: 4 },
